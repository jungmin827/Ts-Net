...
- [x] 통신사 서브페이지 4종 (`/kt` `/sk` `/lg` `/skylife`) — SSG, 미등록 slug는 404
  + 메인 통신사 4카드 허브. 요금제는 `plans` 테이블 기반, 미등록 시 상담 유도로 대체
  - 요금제 데이터 입력 필요: `BE/supabase/seed/plans_template.sql` (자리표시자 → 실제 값)
- [ ] 후기·공지 게시판
- [ ] SEO·트래킹·도메인 연결
